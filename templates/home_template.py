...
class FeedBackHelper:
    ...
    def UploadNewDataMenu(self):
      ...
      if old_data is not None:
         old_data = pd.DataFrame(old_data, columns=['idx'] + Database_Manager.COLUMNS_FOR_CREATION)
         #st.write(old_data['Reservation: Venue'].unique().tolist())
      files = st.file_uploader("Upload Excel", type="xlsx", accept_multiple_files=True, key = 'upload_3')
      ...
      if len(files) > 0:
         ...
         files = [f for f in files if f.name != 'Direct_Feedback.xlsx']
         # readin the data
         # increase progress bar to 10
         my_bar.progress(10, text='Reading Data')
         data = add_additional_data_from_uploaded_file(old_data, files)
         # increase progress bar to 20
         my_bar.progress(20, text='Process data')
         self.df = self.process_data(data)
         # transform the data
         # increase progress bar to 30
         my_bar.progress(30, text='Saving Data')
         self.save_to_db('pages/details.db')
         # save to database
         # increase progress bar to 40
         # update the database
         my_bar.progress(40, text='Updating Application')
         self.db_main_manager.create_database_for_each_venue()
         st.success('Data uploaded successfully to {}'.format(self.db_name))
